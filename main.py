...
# Config 
class Config(object):
    BOT_TOKEN = os.environ.get("BOT_TOKEN", "7616931264:AAGcY8COR_H6ZAz2J75EmFufKC416WPE2XI")
    API_ID = int(os.environ.get("API_ID",  "29773843"))
    API_HASH = os.environ.get("API_HASH", "8b40e91c29a00fecb905d6eb6aee2b3b")
    DOWNLOAD_LOCATION = "./DOWNLOADS"
    SESSIONS = "./SESSIONS"

    AUTH_USERS = os.environ.get('AUTH_USERS', '6350117077').split(',')
    for i in range(len(AUTH_USERS)):
        AUTH_USERS[i] = int(AUTH_USERS[i])

    GROUPS = os.environ.get('GROUPS', '-1002493840482').split(',')
    for i in range(len(GROUPS)):
        GROUPS[i] = int(GROUPS[i])

    LOG_CH = os.environ.get("LOG_CH", "-1002448672568")
